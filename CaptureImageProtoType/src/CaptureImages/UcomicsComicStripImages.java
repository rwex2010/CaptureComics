...
public class UcomicsComicStripImages extends ComicStripImages {
    ...
    private UcomicsCreateImageUrl ucomicsFindImageUrl;
    ...
    public UcomicsComicStripImages(DisplayDebugMessage dbgDisplay) {
	super(dbgDisplay);
	...
	try {
	    ucomicsFindImageUrl = new UcomicsCreateImageUrl(this.dbgDisplay);
	} catch (Exception e) {
	    intDebugCode = 1;
	    strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages -> initializeClassVariables - unable to instantiate UcomicsCreateImageUrl:";
	...
	intDebugCode = 2;
	String strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages -> stepThroughDates - starting";
	dbgDisplay.ShowMessage(strDebugMsg, intDebugCode);
	this.strComicCode = csData.ComicCode;
	while (!(perDateRange.isNegative()) && intLoopLimit > 0) {

	    intDebugCode = 2;
	    strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages -> stepThroughDates while loop ";
		...
		intDebugCode = 2;
		strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages->stepThroughDates blnOkayToContinue is true ";
		strDebugMsg += NL + " Domain: " + csData.Domain;
		strDebugMsg += NL + " comic code: " + strComicCode;
		dbgDisplay.ShowMessage(strDebugMsg, intDebugCode);
		ucomicsFindImageUrl.setCsData(csData);
		try {
		    ucomicsFindImageUrl.BuildImageUrl(ldWorkingDate, csData);
		    captureComicImage();
		} catch (Exception e) {
		...
		intDebugCode = 4;
		strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages->stepThroughDates after UNsuccessful bitwise compare";
	...
	this.strImageUrl = ucomicsFindImageUrl.getStrImageUrl();
	this.strImageId = ucomicsFindImageUrl.getStrImageId();
	this.strImageExtention = ucomicsFindImageUrl.getStrImageExtension();

	intDebugCode = 2;
	strDebugMsg = NL + intDebugCode + ") UcomicsComicStripImages->runCaptureImage - starting";
	strDebugMsg += NL + " Image Url: " + this.strImageUrl;
	strDebugMsg += NL + " Image Id: " + this.strImageId;
	strDebugMsg += NL + " Image Extension: " + this.strImageExtention;
	dbgDisplay.ShowMessage(strDebugMsg, intDebugCode);
